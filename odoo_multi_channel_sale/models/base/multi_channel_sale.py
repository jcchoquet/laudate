...
from ...tools import DomainVals, ReverseDict,chunks
...
class MultiChannelSale(models.Model):
    ...
    def get_quantity(self, obj_pro):
        ...
        elif self.channel_stock_action =="free":
            quantity = qty[obj_pro.id]['free_qty']
        else:
        ...
        if not record:
            ...
            if not record:
                if 'product_template_attribute_value_ids' in vals and 'product_tmpl_id' in vals:
                    ...
                    # if oe_env.search([('product_tmpl_id','=',vals['product_tmpl_id']),
                    #   ('product_template_attribute_value_ids','in', _ids)]
                    # ):
                    #   record = oe_env.search([('product_tmpl_id','=',vals['product_tmpl_id']),
                    #       ('product_template_attribute_value_ids','in', _ids)]
                    #   )
                    # Will code for this if required
        return record
    ...
    def cron_feed_evaluation(self):
        for object_model in  ["product.feed","order.feed","category.feed","partner.feed"]:
            ...
            if records:
                for items in chunks(records,size = records[0].channel_id.api_record_limit):
                    items.import_items()
                    self._cr.commit()
        return True
