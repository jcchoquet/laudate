...
class ProductFeed(models.Model):
    ...
    def _create_feed(self,product_data):
        ...
        try:
            if feed_id:
                # _logger.info("================== feed res %r", feed_id)
                feed = self.browse(feed_id)
                feed.write({'feed_variants':[(5,0)], 'state':'draft'})
                feed.write(product_data)
            else:
                feed = self.create(product_data)
        except Exception as e:
            # _logger.info("====== product vals %r", product_data)
            _logger.error(
        ...
        #       store_id = str(variant_data.get('store_id'))
        #       feed_id = self._context.get('variant_feeds').get(channel_id,{}).get(store_id)
        #       try:
        #           if feed_id:
        #               self.env['product.variant.feed'].browse(feed_id).write(variant_data)
        #           else:
    ...
    def update_product_variants(self,variant_ids,template_id,store_id,location_id,channel_id):
        ...
        for variant in variant_objs:
            ...
            if not exists:
                ...
                exists.product_name.write({'default_code':variant.default_code,'barcode':variant.barcode})
    ...
    def _create_product_line(self,variant,template_id,store_id,location_id,channel_id):
        ...
        if not exists:
            ...
            if not product_exists_odoo:
                ...
                # match = self._context.get('variant_mappings').get(channel_id.id,{}).get(variant_id)
    ...
    def check_attribute_value(self,variant_ids):
        ...
        for variant in variant_ids:
            # _logger.info("==================feed variant val %r", [variant.name_value, variant, variant.store_id])
    ...
    def import_product(self,channel_id):
        ...
        _logger.info("variant_lines + store_id ============> %r",[variant_lines, store_id])
        feed_variants = self.feed_variants
        if variant_lines:
            check_attr = self.check_attribute_value(feed_variants)
            # _logger.info("check_attr =================> %r",check_attr)
        ...
        # _logger.info("template_exists_odoo ===========> %r",template_exists_odoo)
        ...
        if state == 'done':
            if match:
                ...
                try:
                    ...
                    # _logger.info("templta eexits odoo ==========> %r",template_exists_odoo)
                    if not template_exists_odoo:
                        if variant_lines:
                        ...
                        if extra_categ_ids:
                            ...
                            template_id = self.env['product.template'].with_context(context).create(vals)
                            # _logger.info("template_id ===========> %r",template_id)
                    else:
                        # template_exists_odoo.with_context(context).write(vals)
                        template_id = template_exists_odoo
                        # _logger.info("template id =======else=========> %r",template_id)
                        extra_categ_ids = vals.pop('extra_categ_ids')
                        if extra_categ_ids:
                            res = self.get_extra_categ_ids(extra_categ_ids,channel_id)
                            message += res.get('message','')
                    ...
                    if len(variant_lines) and template_id:
                        # _logger.info("variant lines =============> %r",variant_lines)
