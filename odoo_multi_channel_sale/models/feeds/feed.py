...
class WkFeed(models.Model):
	...
	def get_product_id(self,store_product_id,line_variant_ids,channel_id ,default_code=None,barcode=None):
		...
		if match:
			...
			# if 'No Variants'==line_variant_ids:
			# 	variant_feed_id = self._context.get('variant_feeds').get(channel_id.id,{}).get(line_variant_ids)
			# 	product_variant_feeds = self.env['product.variant.feed'].browse(variant_feed_id)
			if feed:
	...
	def contextualize_feeds(self,type,channel_ids=False):
		...
		_logger.info("=========contextualize feeds %r", channel_ids)
		if not channel_ids:
		...
		if type == 'category':
			...
			variant_feeds = {}
			for rec in self.env['product.variant.feed'].search_read(
				[('channel_id','in',channel_ids)],
				['id','store_id','channel_id'],
			):
				variant_feeds.setdefault(rec.get('channel_id')[0],{})[rec.get('store_id')] = rec.get('id')
			return self.with_context(product_feeds=product_feeds,variant_feeds=variant_feeds)
		elif type == 'partner':
			feeds = {}
	...
	def contextualize_mappings(self,type,channel_ids=False):
		...
		_logger.info("=========contextualize mappings %r", channel_ids)
		if type == 'category':
			mappings = {}
			...
			for rec in self.env['channel.product.mappings'].search_read(
				...
				variant_mappings.setdefault(rec.get('channel_id')[0],{}).setdefault(rec.get('store_product_id'),{})[rec.get('store_variant_id')] = rec.get('id')
